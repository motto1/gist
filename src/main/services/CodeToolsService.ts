...
class CodeToolsService {
  ...
  public async getBunPath() {
    const dir = path.join(os.homedir(), '.readnomore', 'bin')
  ...
  private async isPackageInstalled(cliTool: string): Promise<boolean> {
    ...
    const binDir = path.join(os.homedir(), '.readnomore', 'bin')
  ...
  public async updatePackage(cliTool: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      const bunInstallPath = path.join(os.homedir(), '.readnomore')
      const registryUrl = await this.getNpmRegistryUrl()
  ...
  async run(
    ...
    switch (platform) {
      ...
      case 'win32': {
        ...
        const tempDir = path.join(os.tmpdir(), 'readnomore')
