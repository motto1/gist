...
const PREVIEW_MAX_LENGTH = 4096 // Read first 4KB for preview
...
class FileStorage {
      ...
      if (storedStats.size === fileSize) {
        ...
        if (originalHash === storedHash) {
          ...
          return {
            ...
            mtime: storedStats.mtime.getTime(),
  ...
  public selectFile = async (
    ...
    const fileMetadataPromises = result.filePaths.map(async (filePath) => {
      ...
      let charLength = 0
      // only calculate charLength for text files
      if (fileType === 'text') {
        try {
          ...
          logger.error(
            `Failed to read and decode file for charLength calculation: ${filePath}`,
            error as Error
          )
        }
      }
      ...
      return {
        ...
        type: fileType,
        count: 1,
        charLength: 0 // for non-text files
      }
    })
