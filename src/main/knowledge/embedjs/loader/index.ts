...
import { type AddLoaderReturn,JsonLoader, LocalPathLoader, type RAGApplication, TextLoader, WebLoader } from '@main/knowledge/rag'
...
export async function addFileLoader(
  ...
  switch (loaderType) {
    ...
    case 'html':
      ...
      loaderReturn = await ragApplication.addLoader(
        new WebLoader({
          urlOrContent: (await readTextFileWithAutoEncoding(filePath)).content,
          chunkSize: base.chunkSize,
          chunkOverlap: base.chunkOverlap
        }) as any,
        forceReload
    ...
    case 'json':
      try {
        jsonObject = JSON.parse((await readTextFileWithAutoEncoding(filePath)).content)
      } catch (error) {
        jsonParsed = false
        logger.warn(
    ...
    default:
      ...
      loaderReturn = await ragApplication.addLoader(
        new TextLoader({
          text: (await readTextFileWithAutoEncoding(filePath)).content,
          chunkSize: base.chunkSize,
          chunkOverlap: base.chunkOverlap
        }) as any,
        forceReload
