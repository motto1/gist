import './services/NovelCompressionService'
import './services/NovelCharacterService'
import './services/NovelCompressionMemoryService'
import './services/NovelOutlineMemoryService'
...
import { registerNovelOutlineHandlers } from './services/NovelOutlineService'
...
import TextReaderCacheService from './services/TextReaderCacheService'
// import textEditorLibraryService from './services/TextEditorLibraryService'
...
const textReaderCacheService = new TextReaderCacheService()
...
export function registerIpc(mainWindow: BrowserWindow, app: Electron.App) {
  ...
  ipcMain.handle(IpcChannel.App_SetLogLevel, (_, level: string) => {
    logger.setLevel(level as any)
    return { success: true, level }
  })

  ipcMain.handle(IpcChannel.App_GetLogLevel, () => {
  ...
  ipcMain.handle(IpcChannel.Fs_Readdir, FileService.readdir.bind(FileService))
  ...
  // novel outline (大纲提取器)
  registerNovelOutlineHandlers(mainWindow)
  // NovelOutlineMemoryService 的 handlers 在构造函数中自动注册
  ...
  // Path handlers
  ipcMain.handle(IpcChannel.Path_Parse, (_, filePath: string) => {
    return path.parse(filePath)
  })
  ipcMain.handle(IpcChannel.Path_Dirname, (_, filePath: string) => {
    return path.dirname(filePath)
  })
  ipcMain.handle(IpcChannel.Path_Join, (_, ...paths: string[]) => {
    return path.join(...paths)
  })

  // TextBooks Utils (图书工具函数)
  ipcMain.handle(IpcChannel.TextBooks_GenerateFolderName, (_, bookTitle: string, existingFolderNames: string[]) => {
    const { sanitizeFolderName } = require('@main/utils/file')
    const safeName = sanitizeFolderName(bookTitle)
  ...
  ipcMain.handle(IpcChannel.TextBooks_GetTextBooksDir, () => {
    const { getTextBooksDir } = require('@main/utils/file')
    return getTextBooksDir()
  })

  // Text Reader Cache (阅读器缓存)
  const ensureInsideTextBooks = (targetPath: string, opts?: { mustInclude?: string }) => {
    const { getTextBooksDir } = require('@main/utils/file')
    const textBooksDir = getTextBooksDir()

    if (!isPathInside(targetPath, textBooksDir)) {
      throw new Error('非法路径：仅允许访问 TextBooks 库内文件')
    }

    if (opts?.mustInclude) {
      const normalized = targetPath.replace(/\\/g, '/')
      if (!normalized.includes(opts.mustInclude)) {
        throw new Error('非法路径')
      }
    }
  }

  ipcMain.handle(IpcChannel.TextReader_OpenBook, async (_, contentPath: string) => {
    ensureInsideTextBooks(contentPath)
    return await textReaderCacheService.openBook(contentPath)
  })

  ipcMain.handle(IpcChannel.TextReader_GetCacheIndex, async (_, contentPath: string) => {
    ensureInsideTextBooks(contentPath)
    return await textReaderCacheService.getCacheIndex(contentPath)
  })

  ipcMain.handle(IpcChannel.TextReader_ReadChapter, async (_, cachePath: string) => {
    // 仅允许读 reader_cache 内生成的章节文件
    ensureInsideTextBooks(cachePath, { mustInclude: '/reader_cache/chapters/' })
    return await textReaderCacheService.readChapter(cachePath)
  })

  ipcMain.handle(IpcChannel.TextReader_RebuildCache, async (_, contentPath: string) => {
    ensureInsideTextBooks(contentPath)
    await textReaderCacheService.rebuildCache(contentPath)
  })

  // TXT Reader (TXT阅读器)
  // ipcMain.handle(IpcChannel.TextEditorLibrary_GetChapters, (_, bookId: string) =>
  //   textEditorLibraryService.getChapters(bookId)
  // )
  // ipcMain.handle(IpcChannel.TextEditorLibrary_SaveChapters, (_, bookId: string, chapters) =>
  //   textEditorLibraryService.saveChapters(bookId, chapters)
  // )
  // ipcMain.handle(IpcChannel.TextEditorLibrary_ReparseChapters, (_, bookId: string) =>
  //   textEditorLibraryService.reparseChapters(bookId)
  // )
