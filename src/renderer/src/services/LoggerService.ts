...
const IS_DEV = IS_WORKER ? false : (typeof window !== 'undefined' && window.electron?.process?.env?.NODE_ENV === 'development')
...
class LoggerService {
  ...
  private constructor() {
    if (IS_DEV && typeof window !== 'undefined' && window.electron) {
      if (
  ...
  private processLog(level: LogLevel, message: string, data: any[]): void {
    ...
    if (currentLevel >= LEVEL_MAP[this.logToMainLevel] || forceLogToMain) {
      ...
      if (!IS_WORKER && typeof window !== 'undefined' && window.api?.logToMain) {
