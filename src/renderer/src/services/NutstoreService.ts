...
async function cleanupOldBackups(webdavConfig: WebDavConfig, maxBackups: number): Promise<void> {
  ...
  try {
    ...
    const backupFiles = files
      .filter((file) => (file.fileName.startsWith('read-no-more') || file.fileName.startsWith('gist')) && file.fileName.endsWith('.zip'))
...
export async function backupToNutstore({
  ...
  const backupFileName = customFileName || `gist.${timestamp}.${deviceType}.zip`
