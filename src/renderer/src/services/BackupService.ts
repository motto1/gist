...
export async function handleData(data: Record<string, any>) {
  if (data.version === 1) {
    ...
    await localStorage.setItem('persist:read-no-more', data.localStorage['persist:read-no-more'])
  ...
  if (data.version >= 2) {
    localStorage.setItem('persist:read-no-more', data.localStorage['persist:read-no-more'])
