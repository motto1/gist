/**
 * Read-No-More AI Core - 新版本入口
 * 集成 @readnomore/ai-core 库的渐进式重构方案
 *
 * 融合方案：简化实现，专注于核心功能
 * 1. 优先使用新AI SDK
 * 2. 暂时保持接口兼容性
...
export default class ModernAiProvider {
  ...
  constructor(modelOrProvider: Model | Provider, provider?: Provider) {
    ...
    if (this.isModel(modelOrProvider)) {
      ...
      tempProvider = provider || getActualProvider(modelOrProvider)
    } else {
      // 传入的是 Provider
      tempProvider = modelOrProvider
    }

    // 如果是系统内置的provider，从原始配置中合并以确保数据完整性
    if (tempProvider.isSystem) {
      const systemProviderConfig = SYSTEM_PROVIDERS_CONFIG[tempProvider.id]
      if (systemProviderConfig) {
        this.actualProvider = { ...systemProviderConfig, ...tempProvider }
      } else {
        this.actualProvider = tempProvider
      }
    } else {
      this.actualProvider = tempProvider
    }
