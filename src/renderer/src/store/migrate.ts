...
const migrateConfig = {
  ...
  '139': (state: RootState) => {
    try {
      addProvider(state, 'readnomore')
      state.llm.providers = moveProvider(state.llm.providers, 'readnomore', 1)
  ...
  '157': (state: RootState) => {
    try {
      ...
      if (!icons.visible.includes('novel_compress') && !icons.disabled.includes('novel_compress')) {
        icons.disabled.push('novel_compress')
      }
      icons.visible = icons.visible.filter((icon) => DEFAULT_SIDEBAR_ICONS.includes(icon))
      icons.disabled = icons.disabled.filter((icon) => DEFAULT_SIDEBAR_ICONS.includes(icon))
      return state
    } catch (error) {
  ...
  '158': (state: RootState) => {
    try {
      addProvider(state, 'fast-read-experience')
      return state
    } catch (error) {
  ...
  '159': (state: RootState) => {
    try {
      // 更新 fast-read-experience 供应商的模型列表
      updateProvider(state, 'fast-read-experience', {
  ...
  '160': (state: RootState) => {
    try {
      // 导入 deepseekV31Model
      const deepseekV31Model: Model = {
        id: 'deepseek-v3.1',
        name: 'deepseek-v3.1',
        provider: 'fast-read-experience',
        group: 'deepseek'
      }

      // 更新系统默认模型为 deepseek-v3.1
      state.llm.defaultModel = deepseekV31Model
      state.llm.topicNamingModel = deepseekV31Model
      state.llm.translateModel = deepseekV31Model
      state.llm.quickModel = deepseekV31Model
