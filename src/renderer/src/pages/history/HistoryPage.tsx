...
import { Button, Divider, Input, InputRef, Tooltip } from 'antd'
import { last } from 'lodash'
import { ChevronLeft, CornerDownLeft, RefreshCw, Search } from 'lucide-react'
...
const HistoryPage: FC = () => {
  ...
  const cachedTopicIds = useAppSelector((state) => Object.keys(state.messages.messageIdsByTopic))
  ...
  const onReload = async () => {
    setIsReloading(true)
    try {
      cachedTopicIds.forEach((topicId) => dispatch(newMessagesActions.clearTopicMessages(topicId)))

      const topicIdsToReload = new Set<string>()
      if (topic?.id) topicIdsToReload.add(topic.id)
      if (message?.topicId) topicIdsToReload.add(message.topicId)

      topicIdsToReload.forEach((topicId) => dispatch(loadTopicMessagesThunk(topicId, true)))
  ...
  return (
    <Container>
      <HStack style={{ padding: '0 12px', marginTop: 8 }}>
        ...
        />
        <Tooltip title={t('history.reload')}>
          <Button
            type="text"
      ...
      <TopicsHistory
        key={`topics-${reloadSeq}`}
        keywords={search}
        onClick={onTopicClick as any}
        onSearch={onSearch}
        style={{ display: isShow('topics') }}
      />
      <TopicMessages key={`topic-${reloadSeq}`} topic={topic} style={{ display: isShow('topic') }} />
      <SearchResults
        key={`search-${reloadSeq}`}
        keywords={isShow('search') ? searchKeywords : ''}
        onMessageClick={onMessageClick}
        onTopicClick={onTopicClick}
        style={{ display: isShow('search') }}
      />
      <SearchMessage key={`message-${reloadSeq}`} message={message} style={{ display: isShow('message') }} />
    </Container>
  )
}
